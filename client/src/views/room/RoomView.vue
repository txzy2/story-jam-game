...
<template>
  <Motion :initial="{ opacity: 0, scale: 0 }" :animate="{ opacity: 1, scale: 1 }">
    <div class="h-[80vh] flex flex-col items-center justify-center gap-2" v-if="!gameStarted">
      <div class="cursor-pointer flex flex-col items-center gap-1 font-bold rounded-sm">
        <h2 class="flex items-center gap-2 text-xl">
          История
          <pre>{{ room.title.toLowerCase() }}</pre>

          <div v-if="userStore.getUser.id === room.ownerId">
            <Button class="h-[30px] cursor-pointer flex items-center justify-center px-2 gap-1 text-[13px] rounded-sm">
              <Trash2 :size="20" />
              Удалить
            </Button>
          </div>
        ...
        <Badge title="Скопировать код комнаты" @click="copyToClipboard(room.code)"
      ...
      <Motion :initial="{ opacity: 0, scale: 0 }" :animate="{ opacity: 1, scale: 1 }">
        <div class="flex items-center gap-1">
          <ul>
            <li v-for="player in room.players" :key="player.userId" class="flex items-center gap-2">
              <img :src="player.avatar" :alt="'Player' + player.avatar" class="w-8 h-8 rounded-full" />
              <div class="flex flex-col items-center leading-none">
                <pre>{{ player.username }}</pre>
                <p v-if="player.userId === room.ownerId" class="text-[12px] font-bold">
                  (Создатель)
                </p>
              </div>
              ...
              " class="p-0" title="Выйти из комнаты" variant="ghost" @click="exitRoom">
                <X :size="20" />
              </Button>
            </li>
          </ul>
      ...
      <Badge v-if="connected !== room?.maxPlayers" class="flex items-center gap-1 px-2 rounded-sm">
      ...
      <Motion :initial="{ opacity: 0, scale: 0 }" :animate="{ opacity: 1, scale: 1 }">
        <Button v-if="
          ...
          userStore.getUser.id === room.ownerId
        " class="h-[30px] flex items-center gap-1" :class="{
            'anim_gradient--dark': colorMode === 'dark',
            'anim_gradient--light': colorMode === 'light'
          }" type="button" @click="startGame">
          <Play :size="20" />
          <p>Начать игру</p>
        </Button>
